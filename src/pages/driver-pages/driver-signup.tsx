...
const DriverSignUp = () => {
	...
	const newDriver: any = localStorage.getItem("newDriver")
		? JSON.parse(localStorage.getItem("newDriver") as string)
		: {
				user_metadata: {},
				bus: {},
				docs: {},
	...
	const [email, setEmail] = useState<string>(
		newDriver?.user_metadata?.email || ""
	);
	const [fName, setFName] = useState<string>(
		newDriver?.user_metadata?.first_name || ""
	);
	...
	const [profile, setProfile] = useState<string>(
		newDriver?.user_metadata?.photo || ""
	);
	const [licenseNumber, setLicenseNumber] = useState<string>(
		newDriver?.user_metadata?.driver_license || ""
	);
	const [registrationNumber, setRegistrationNumber] = useState<string>(
		newDriver?.bus?.registration_number || ""
	);
	const [locationName, setlocationName] = useState(
		newDriver?.user_metadata?.location || ""
	);
	const [location, setlocation] = useState(false);
	const [make, setMake] = useState(newDriver?.bus?.make || "");
	...
	const [accountNo, setAccountNo] = useState(
		newDriver?.account?.account_number || ""
	);
	const [bankFilter, setBankFilter] = useState("");
	const [vehicleCapacity, setVehicleCapacity] = useState(
		newDriver?.bus?.capacity || 0
	);
	...
	const [proofOfInsurance, setProofOfInsurance] = useState<string>(
		newDriver?.bus?.bus_insurance || ""
	);
	const [roadWorthiness, setRoadWorthiness] = useState<string>(
		newDriver?.bus?.road_worthiness_cert || ""
	);
	...
	const handleNext = () => {
		...
		if (currentPage === 3) {
			if (!driverLicense || !proofOfInsurance || !roadWorthiness) {
				setShowError(true);
	...
	const handleSubmit = () => {
		...
		dispatch(registerAsADriverAction({ ...newDriver?.user_metadata }))
			.then(() => {
				dispatch(
					createBusAction({
						driver: userInfo?._id,
