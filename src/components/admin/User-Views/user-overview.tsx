				...
				<table className="w-full text-left text-white">
					<thead className="bg-black">
						<tr className="w-full">
							<th
								scope="col"
								className="px-4 py-4 font-normal text-sm rounded-l-md">
								First Name
							</th>
							<th
								scope="col"
							...
							<th
								scope="col"
								className="px-2 py-4 font-normal text-sm rounded-r-md"></th>
						</tr>
					</thead>
			...
			{flip === "activate" && modalVisible && (
				<Modal
					...
					<Button
						title="Activate"
						type="submit"
						className="w-full py-2 mt-8 text-sm rounded-md bg-[#00FF6A]"
						onClick={() => {
							//LEKAN
							dispatch(blockUserAction(selectedUser?._id || ""));
							setFlip("success");
